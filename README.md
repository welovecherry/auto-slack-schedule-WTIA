...
WTIA 프로그램은 약 두 달간 진행됨. 매일의 일정은 파워포인트 안에 들어 있고, 그게 슬랙 채널에 올라옴. 오늘 일정을 알려면 약 30명 각자가 슬랙을 열고 → 채널로 가서 → 파워포인트를 열고 → 오늘 날짜를 찾아야 했음. 작지만 매일 아침 반복되는 번거로운 작업임

이 봇은 그 번거로움을 모두를 위해 없앰. 매일 아침 오늘의 일정(시간·활동·장소)을 텍스트로 슬랙 채널에 올려줌. 파일을 열 필요 없이 채널만 보면 됨.

---

## 무엇을 하나

- 매일 아침 **오전 7시쯤(태평양 시간)** 알아서 실행됨.
- 일정 파일에서 **오늘 날짜**를 찾음.
- 그날의 일정을 슬랙 채널에 올림.
- 수업이 없는 날은 아무것도 올리지 않음.

버튼을 누를 필요 없음. 내 컴퓨터가 꺼져 있어도 계속 돌아감.

---

## 동작 원리

전체는 네 조각이 함께 일하는 구조임:
...
쉽게 말하면, GitHub에는 내장 알람시계(**cron**)가 있음. 정해진 시각에 깨어나 GitHub 클라우드의 작은 컴퓨터를 몇 초간 빌려 스크립트를 돌리고 끔. 스크립트는 오늘 날짜를 확인해 맞는 일정을 찾고, **웹훅**(채널에 메시지를 넣을 수 있는 비공개 링크)을 통해 슬랙으로 보냄.
...
| 파일 | 종류 | 역할 |
|------|------|------|
| `schedule.json` | 데이터 | 실제 일정 — 날짜별 시간·활동·장소. **평소에 수정하는 건 이 파일 하나뿐임.** |
| `send_schedule.py` | 파이썬 코드 | 일꾼. 오늘 일정을 찾아 슬랙으로 보냄. |
| `.github/workflows/daily-slack.yml` | 설정 | GitHub에게 *언제* 무엇을 실행할지 알려줌. |
...
핵심 설계: **데이터**(`schedule.json`)와 **코드**(`send_schedule.py`)의 분리. 일정이 바뀌면 데이터 파일만 고치면 되고, 코드는 읽거나 손댈 필요 없음.
...
1. **슬랙 웹훅 만들기.** https://api.slack.com/apps → *Create New App* → *From scratch*. **Incoming Webhooks**를 켜고 *Add New Webhook to Workspace* → 채널 선택 → URL 복사. 이 URL은 비밀로 보관할 것(가진 사람은 누구나 채널에 글을 쓸 수 있음).
2. **비공개(private) GitHub 저장소 만들기.**
3. 위 **파일 3개 추가.** `.yml` 파일은 반드시 `.github/workflows/` 안에 있어야 함.
...
GitHub에서 `schedule.json`을 열고 연필(Edit)을 눌러 날짜를 추가·수정. 하루의 형식:
...
- 날짜 키는 `YYYY-MM-DD` 형식.
- `time`, `location`은 선택사항. `time`을 빼면 시간 없이 표시됨.
- 파일에 **없는** 날짜는 그날 **아무것도** 안 보냄.

*Commit changes*를 누르면 끝.

**아침까지 안 기다리고 특정 날 테스트:** *Actions → Run workflow*에서 날짜(예: `2026-06-24`) 입력.

---

## 발송 시간 바꾸는 법 (cron)

시간은 `.github/workflows/daily-slack.yml`의 한 줄에 있음:
...
**중요:** GitHub은 **UTC 시간만** 씀(현지 시간 아님). 그래서 현지 시간을 UTC로 변환해야 함.
...
솔직한 참고사항:
- GitHub은 몇 분(5~20분) 늦게 실행될 수 있음. "7시 정각"이 아니라 "7시쯤"이 맞는 기대치임.
- 가을에 태평양이 표준시(PST)로 바뀌면 같은 UTC 시각이 한 시간 일찍 잡힘. 이 프로그램은 8월에 끝나므로 해당 없음.
- 저장소가 **60일간 활동이 없으면** GitHub이 스케줄을 끔. 아무 커밋이나 하나 넣으면 다시 깨어남.
